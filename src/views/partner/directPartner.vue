...
<template>
  <el-card class="next_container">
    <div class="page_title">
      <span class="line"></span>{{ $t("direct_partner.title") }}
    </div>
    ...
    <el-table :data="tableData" v-loading="tableLoading" style="width: 100%" :row-style="rowStyle">
      <el-table-column :label="$t('direct_partner.account')" align="center">
        <template slot-scope="scope">
          {{ scope.row.email || scope.row.mobileNumber }}
        </template>
      </el-table-column>
      <!-- <el-table-column prop="rate" label="上级返佣比例(%)" align="center" /> -->
      <el-table-column :label="$t('direct_partner.identity')" align="center">
        <template slot-scope="scope">
          {{
      ...
      <el-table-column prop="partnerUid" :label="$t('invite_uid')" width="150" align="center" />
      ...
      <el-table-column :label="$t('direct_partner.commission_ratio')" width="150" align="center">
        <template slot-scope="scope">
          {{ scope.row.rate > 0 ? `${scope.row.rate}%` : `--` }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('register_date')" prop="regesterTime" align="center" :formatter="formatTime" />
      ...
      <el-table-column :label="$t('operate')" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button type="text" @click="changeRemark(scope.row)">{{
            $t("direct_partner.edit_remark")
          }}</el-button>
          <el-button v-if="scope.row.rate > 0" type="text" @click="changeRate(scope.row)">{{
            $t("direct_partner.set_commission_ratio") }}</el-button>
          <el-button v-if="scope.row.rate == 0" type="text" @click="changeRate(scope.row)">{{
            $t("direct_partner.upgrade_partner") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogVisible['rate']" custom-class="rate_mask" width="480px" center>
      <div class="rate_title" slot="title">
        <div class="title">
          <span class="point"></span>{{ $t("direct_partner.set_commission_ratio_title") }}
        </div>
      </div>
      <div class="rate_main">
        <div class="input_line">
          <div class="label">{{ $t("direct_partner.ratio") }}：</div>
          <div class="input_box">
            <el-input style="width: 325px;" type="tel" :placeholder="$t('direct_partner.enter_ratio_placeholder')"
              v-model="rateFormData.rate" class="rate_input">
              <template slot="append">%</template>
            </el-input>
          </div>
        </div>
        <div class="tips">
          <span class="el-icon-warning-outline"></span>{{ $t("direct_partner.about_ratio") }}
        </div>
        <div class="btn_line">
          <el-button class="cancel_btn" @click="dialogVisible['rate'] = false">
            <span>{{ $t("cancel") }}</span></el-button>
          <el-button class="submit_btn" :loading="loading" @click="submitRate">
            <span>{{ $t("confirm") }}</span></el-button>
        </div>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    changeRate (row) {
      ...
      this.limitRate = row.rate;
      this.$set(this.dialogVisible, "rate", true);
    },
    changeRemark (row) {
      this.remarkFormData = {
        id: row.id,
    ...
    submitRate () {
      if (Number(this.rateFormData.rate) < this.limitRate) {
        this.$message({
          message: this.$t("direct_partner.ratio_decreased"),
          type: "info",
          duration: 2 * 1000
        });
        return;
      ...
      if (
        Math.floor(this.rateFormData.rate) !== Number(this.rateFormData.rate)
      ) {
        this.$message({
          message: this.$t("direct_partner.ratio_integer"),
          type: "info",
          duration: 2 * 1000
        });
        return;
      ...
      if (
        Number(this.rateFormData.rate) > 100 ||
        Number(this.rateFormData.rate) < 0
      ) {
        this.$message({
          message: this.$t('direct_partner.ratio_in_100'),
          type: "info",
          duration: 2 * 1000
        });
        return;
      ...
      this.loading = true;
      this.rateFormData.rate = Number(this.rateFormData.rate);
      Api.updateRate(this.rateFormData)
        .then(res => {
          this.$message({
            message: this.$t('successfully_set'),
            type: "success",
            duration: 2 * 1000,
            onClose: () => {
              this.$set(this.dialogVisible, "rate", false);
              this.loading = false;
              this.getTableData();
            }
          });
    ...
    getTableData () {
      ...
      //   }
      // });
      Api.getDirectPartnerNext(this.baseInfo).then(res => {
...
.next_container {
  ...
  .rate_mask {
    ...
    .rate_main {
      padding: 24px;
      box-shadow: 4px 4px 20px 0px rgba(205, 205, 205, 0.2);
      border-radius: 8px;
