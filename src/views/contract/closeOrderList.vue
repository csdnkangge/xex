...
<template>
  <el-card class="close_order_list_container">
    <div class="page_title">
      <span class="line"></span>{{ $t("close_order_list.title") }}
    </div>
    <el-form :inline="true" label-position="top">
      ...
      <el-form-item :label="$t('close_order_list.order_id')" class="form-item">
        <el-input
          v-model="baseInfo.id"
          :placeholder="$t('close_order_list.order_id')"
          clearable
        />
      </el-form-item>
      <el-form-item
        :label="$t('close_order_list.contract_name')"
      ...
      >
        <el-select
          ...
          filterable
          clearable
          :placeholder="$t('close_order_list.all')"
        >
          <el-option
      ...
      <el-form-item
        :label="$t('close_order_list.update_time')"
    ...
    >
      <!-- 订单id -->
      <el-table-column
        :label="$t('close_order_list.order_id')"
        prop="id"
        align="center"
        width="80"
      >
      ...
      <!-- 仓位类型 -->
      <el-table-column
        :label="$t('close_order_list.position_type')"
        align="center"
        width="120"
      >
        <template slot-scope="scope">
          {{
            scope.row.position_type === 1
              ? $t("close_order_list.cross")
              : scope.row.position_type === 2
              ? $t("close_order_list.isolated")
              : $t("close_order_list.unknow")
          }}
        </template>
      </el-table-column>
      <!-- 结算盈亏 -->
      <el-table-column
        :label="$t('close_order_list.set_profit_loss')"
        align="center"
        prop="realized_amount"
        width="180"
      />
      <!-- 方向 -->
      <el-table-column :label="$t('close_order_list.direction')" width="120" align="center">
        <template slot-scope="scope">
          <p
            :class="{
      ...
      <!-- 杠杆 -->
      <el-table-column :label="$t('close_order_list.lever')" prop="leverage_level" align="center" />
      <!-- 平仓价格 -->
      <el-table-column :label="$t('close_order_list.close_price')" width="140" prop="price_txt" align="center" />
      <!-- 平仓数量 -->
      <el-table-column
        :label="$t('close_order_list.closing_quantity')"
        prop="pc_volume_txt"
        width="170"
        align="center"
      />
      <!-- 手续费 -->
      <el-table-column :label="$t('close_order_list.fee')" prop="trade_fee_txt" align="center" />
      <!-- 更新时间 -->
      <el-table-column
        :label="$t('close_order_list.update_time')"
        prop="mtime"
      ...
      Api.getContractOrderList(data).then(res => {
        if (res.data.data && res.data.data.length > 0) {
          this.tableData = res.data.data.map(item => {
            ...
            item.avgPrice =
              item.avg_price === 0
                ...
                : fixD(item.avg_price, item.price_precision || 2) +
                  ` ${this.setUnit(item.symbol, 1)}`;
            item.price_txt = null;
            if (item.type === 2 && Number(item.price) === 0) {
              // item.price_txt = "市价"; // '市价';
              item.price_txt = item.avgPrice;
            } else {
              item.price_txt =
            ...
            item.pc_volume_txt = deal_volume;
