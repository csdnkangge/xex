...
<template>
  <el-card class="otc_container">
    <div class="page_title">
      <span class="line"></span>{{ $t("fiat_asset.title") }}
    </div>
    ...
    <el-table
      ...
      :row-style="rowStyle"
    >
      <el-table-column :label="$t('fiat_asset.account')" align="center">
      ...
      >
        <template slot-scope="scope">{{
          scope.row.partnerUid ? scope.row.partnerUid : "--"
        }}</template>
      </el-table-column>
      <el-table-column
        prop="total"
        :label="$t('fiat_asset.total_assets')"
        width="200"
        align="center"
      />
      <el-table-column
        prop="free"
        :label="$t('fiat_asset.available')"
        width="200"
        align="center"
      />
      <el-table-column
        prop="frozen"
        :label="$t('fiat_asset.freeze')"
        width="200"
        align="center"
      />
      ...
      <el-table-column :label="$t('operate')" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            type="text"
            class="detail_btn"
            @click="handleToOTCDetail(scope.row)"
            >{{ $t("fiat_asset.trade_detail") }}</el-button
          >
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      Api.getOTCAccount(data).then(res => {
        this.tableData = res.data.data;
        this.total = res.data.rows;
        this.tableLoading = false;
      });
    },
    handleToOTCDetail(row) {
      this.$router.push({
