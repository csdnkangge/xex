...
<template>
  <el-card class="withdraw_container">
    <div class="page_title">
      <span class="line"></span>{{ $t("withdraw_list.title") }}
    </div>
    <el-form :inline="true" label-position="top">
      ...
      <el-form-item
        :label="$t('withdraw_list.trade_date')"
    ...
    <div class="total_info">
      <div class="total_amount info-item">
        {{ $t("withdraw_list.total") }}：<span class="DP-Bold">{{
          sumUsdt
        }}</span>
    ...
    <el-table
      ...
      :row-style="rowStyle"
    >
      <el-table-column :label="$t('withdraw_list.account')" align="center">
      ...
      <el-table-column
        prop="amount"
        :label="$t('withdraw_list.withdraw_amount')"
        width="200"
        align="center"
      />
      <el-table-column
        :label="$t('withdraw_list.withdraw_state')"
        align="center"
      >
        <template slot-scope="scope">
          {{
            scope.row.status === 0
              ? $t("withdraw_list.unchecked")
              : scope.row.status === 1
              ? $t("withdraw_list.passed")
              : scope.row.status === 2
              ? $t("withdraw_list.rejected")
              : scope.row.status === 3
              ? $t("withdraw_list.remitting")
              : scope.row.status === 4
              ? $t("withdraw_list.payment_failed")
              : scope.row.status === 5
              ? $t("withdraw_list.completed")
              : scope.row.status === 6
              ? $t("withdraw_list.revoked")
              : $t("withdraw_list.unknow")
          }}
        </template>
      </el-table-column>

      <el-table-column
        :label="$t('withdraw_list.withdrawal_time')"
        prop="createTime"
        align="center"
        :formatter="formatTime"
      />
      <el-table-column
        :label="$t('withdraw_list.completed_time')"
        prop="completedTime"
        align="center"
        :formatter="formatTime"
      />
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      Api.getWithdrawList(data).then(res => {
